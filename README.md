# BESQL 在线工具箱

> 44+ 款免费在线工具，覆盖开发、设计、办公全场景。SQL 格式化、JSON 美化、Base64 编解码、时间戳转换、正则测试、图片压缩/裁剪/格式转换、PDF 合并/拆分/加解密、密码生成、Hash 计算等一应俱全。无需注册，无广告，客户端处理保护隐私。访问 **besql.cn** 立即使用

**https://www.besql.cn**

[![License](https://img.shields.io/github/license/your-org/besql?color=blue)](LICENSE)
[![Python](https://img.shields.io/badge/python-3.9+-blue)](https://www.python.org/)

---

## 工具一览

| 分类 | 工具 | 地址 |
|------|------|------|
| **开发工具** | SQL 格式化 | https://www.besql.cn/sql |
| | JSON 格式化 | https://www.besql.cn/json |
| | URL 编码解码 | https://www.besql.cn/url |
| | Base64 编码解码 | https://www.besql.cn/base64 |
| | 时间戳转换 | https://www.besql.cn/timestamp |
| | 正则表达式测试 | https://www.besql.cn/regex |
| | 文本对比 | https://www.besql.cn/diff |
| | 进制转换 | https://www.besql.cn/radix |
| | UUID 生成 | https://www.besql.cn/uuid |
| | Hash 计算 | https://www.besql.cn/hash |
| | 图片 Base64 转换 | https://www.besql.cn/image2base64 |
| **文本工具** | 加密解密 | https://www.besql.cn/encrypt |
| | 去重排序 | https://www.besql.cn/dedupe |
| | 字数统计 | https://www.besql.cn/wordcount |
| | 摩斯密码 | https://www.besql.cn/morse |
| | 简繁转换 | https://www.besql.cn/s2t |
| **图片工具** | 图片压缩 | https://www.besql.cn/img/compress |
| | 图片大小修改 | https://www.besql.cn/img/resize |
| | 图片裁剪 | https://www.besql.cn/img/crop |
| | 图片格式转换 | https://www.besql.cn/img/format |
| | 图片加水印 | https://www.besql.cn/img/watermark |
| | 图片旋转翻转 | https://www.besql.cn/img/rotate |
| **PDF 工具** | PDF 合并 | https://www.besql.cn/pdf/merge |
| | PDF 拆分 | https://www.besql.cn/pdf/split |
| | PDF 旋转 | https://www.besql.cn/pdf/rotate |
| | PDF 加水印 | https://www.besql.cn/pdf/watermark |
| | PDF 去水印 | https://www.besql.cn/pdf/unwatermark |
| | 图片转 PDF | https://www.besql.cn/pdf/image2pdf |
| | PDF 转图片 | https://www.besql.cn/pdf/2image |
| | PDF 文字提取 | https://www.besql.cn/pdf/text |
| | PDF 压缩 | https://www.besql.cn/pdf/compress |
| | PDF 加密 | https://www.besql.cn/pdf/protect |
| **生成与计算** | 密码生成 | https://www.besql.cn/password |
| | 密码强度检测 | https://www.besql.cn/strength |
| | 随机数生成 | https://www.besql.cn/random |
| | 日期计算 | https://www.besql.cn/date |
| | 整数分区计算 | https://www.besql.cn/partition |
| | BMI 计算 | https://www.besql.cn/bmi |
| | 条形码生成 | https://www.besql.cn/barcode |
| **趣味与测试** | 在线秒表 | https://www.besql.cn/stopwatch |
| | 屏幕键盘测试 | https://www.besql.cn/screentest |

## 特点

- **免费无需注册** — 打开网页即用，无广告水印
- **隐私优先** — 图片/文档处理全程在浏览器内完成，不上传服务器
- **大整数 JSON** — 无损解析超过 JavaScript 安全整数范围的超大数字
- **PDF 批量处理** — 合并、拆分、压缩、加解密、水印一站式解决
- **AI 智能提取** — PDF 文字提取支持 OCR，自动识别扫描件内容
- **移动端适配** — 响应式布局，手机平板均可流畅使用

## 技术栈

```
Flask + Gunicorn + Nginx
纯前端图片/PDF处理（Client-side）
HTML + CSS + Vanilla JS
```

## 快速部署

```bash
# 1. 克隆
git clone https://your-org/besql.git && cd besql

# 2. 安装依赖
pip install -r requirements.txt

# 3. 启动
python app.py
```

生产环境请参考 [DEPLOYMENT.md](DEPLOYMENT.md)。

## 项目结构

```
besql/
├── app.py              # Flask 应用入口，路由与 SEO 资源
├── tools/              # 各工具模块（每工具一个 blueprint）
├── templates/          # Jinja2 模板
├── static/             # 静态资源
├── scripts/            # 运维脚本（索引推送、日志分析）
├── tests/              # 测试套件
├── DEPLOYMENT.md       # 阿里云部署指南
└── requirements.txt    # Python 依赖
```

## SEO & 搜索引擎接入

- **百度** — 主动推送 API + 自动推送 JS
- **Bing** — IndexNow 密钥验证 + 自动提交脚本
- **神马 / 360 / 头条 / 搜狗** — robots.txt 分组 + 站点验证
- **全站** — Sitemap XML、FAQPage 结构化数据、OpenGraph / Twitter Card

## License

MIT

---

> 🌐 **https://www.besql.cn** — 用完即走，无负担。
